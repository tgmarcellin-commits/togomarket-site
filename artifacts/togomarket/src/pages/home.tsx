...
import { useGetListings, getGetListingsQueryKey, useGetStats } from "@workspace/api-client-react";
...
export default function Home() {
  ...
  const { data: listings, isLoading } = useGetListings({ search, sector }, { query: { queryKey: getGetListingsQueryKey({ search, sector }) }});
    ...
    <div className="min-h-[100dvh] flex flex-col bg-background pb-20">
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="font-bold text-2xl tracking-tight flex items-center gap-1 cursor-pointer" onClick={() => { setSector(undefined); setSearch(""); setSearchInput(""); }}>
            <span className="text-foreground">Togo</span>
            <span className="text-primary">Market</span>
          </div>
          <Button onClick={() => setIsPublishModalOpen(true)} className="bg-primary hover:bg-primary/90 rounded-full font-semibold px-6">
            Vendre
          </Button>
        </div>
      </header>
      ...
      <section className="relative h-[300px] sm:h-[400px] w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-2xl px-4 text-center">
          ...
          <form onSubmit={handleSearchSubmit} className="relative flex items-center max-w-xl mx-auto">
            ...
            />
            <Button type="submit" className="absolute right-1.5 h-11 rounded-full bg-accent hover:bg-accent/90 text-accent-foreground px-6 font-semibold">
              Chercher
            </Button>
          </form>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto">
              Nous n'avons trouvé aucun article correspondant à votre recherche. Essayez d'autres mots-clés ou commandez-le !
            </p>
          </div>
        )}
      </main>
      ...
      <footer className="mt-auto border-t bg-card py-8 pb-32">
        <div className="container mx-auto px-4 text-center">
          <p className="text-muted-foreground text-sm mb-4">© 2026 TogoMarket. Tous droits réservés.</p>
          <button 
      ...
      {/* Fixed Bottom Bar */}
      <div className="fixed bottom-0 left-0 w-full p-4 bg-background/80 backdrop-blur-md border-t z-50">
        <div className="container mx-auto max-w-3xl">
          <Button 
            onClick={() => setIsOrderModalOpen(true)}
            className="w-full h-14 text-base font-bold rounded-2xl shadow-xl shadow-secondary/20 bg-secondary hover:bg-secondary/90 text-white"
          >
            Vous ne trouvez pas votre article ? Commandez ici !
          </Button>
        </div>
      </div>
