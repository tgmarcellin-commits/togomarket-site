...
export function AuthModal({ open, onOpenChange, onLoginSuccess }: AuthModalProps) {
  ...
  const handleSendWhatsApp = () => {
    ...
    const msg = lang === "fr"
      ? `Bonjour ! Je souhaite activer mon compte vendeur TogoMarket.\nNom : ${verifyInfo.firstName}\nNuméro : ${verifyInfo.phone}\nCode de vérification : ${verifyInfo.verifyCode}`
      : `Hello! I want to activate my TogoMarket seller account.\nName: ${verifyInfo.firstName}\nNumber: ${verifyInfo.phone}\nVerification code: ${verifyInfo.verifyCode}`;
        ...
        {screen === "verify" && verifyInfo && (
          <div className="space-y-5 pt-2">
            <div className="text-center space-y-2">
              ...
              <h3 className="font-semibold text-lg">{t.accountCreated}</h3>
              <p className="text-sm text-muted-foreground">
                {t.activateAccountDesc(verifyInfo.phone)}
              </p>
            </div>

            <div className="bg-muted rounded-xl p-5 text-center">
              <p className="text-xs text-muted-foreground mb-1 uppercase tracking-wider">{t.verifyCodeLabel}</p>
